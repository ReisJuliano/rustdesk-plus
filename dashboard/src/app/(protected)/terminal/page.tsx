...
import {
  listTags, listDevices, execCommand, getExecResults,
  type Tag, type Device, type ExecJobResult,
} from "@/lib/api";
...
export default function TerminalPage() {
  ...
  async function onRun() {
    ...
    try {
      const body: Parameters<typeof execCommand>[0] = {
        cmd: cmd.trim(),
        powershell,
      };
      ...
      if (resp.sent === 0) {
        throw new Error(
          resp.targets === 0
            ...
            : "Nenhum agente está conectado. Baixe e execute novamente o instalador no PC."
        );
      }
                ...
                <button key={m} onClick={() => setTargetMode(m)}
                  className={`px-4 py-1.5 rounded-2xl text-xs font-semibold border transition-all ${
                    targetMode === m
                      ...
                      : "border-slate-200 text-slate-500 hover:bg-slate-50 bg-transparent"
                  }`}>
                  {labels[m]}
                </button>
              );
          ...
          {targetMode === "devices" && (
            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 border border-slate-200 rounded-2xl bg-white">
              {onlineDevices.length === 0 && (
                <p className="text-xs text-slate-400">Nenhum dispositivo online com agente</p>
              )}
              {onlineDevices.map((d) => (
          ...
          <div className="flex gap-3">
            {[{ val: false, label: "CMD" }, { val: true, label: "PowerShell" }].map(({ val, label }) => (
              <label key={label} className="flex items-center gap-1.5 cursor-pointer">
                <input type="radio" checked={powershell === val} onChange={() => setPowershell(val)}
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
            <button
              onClick={onRun}
