...
import { getStats, listDevices, type Stats, type Device } from "@/lib/api";

const statCards = [
  {
    key: "total_devices" as keyof Stats,
    label: "Dispositivos",
    bg: "bg-blue-50",
    iconBg: "bg-blue-600",
  ...
  {
    key: "online_devices" as keyof Stats,
    label: "Online agora",
    bg: "bg-emerald-50",
  ...
  {
    key: "offline_devices" as keyof Stats,
    label: "Offline",
    bg: "bg-slate-100",
    iconBg: "bg-slate-400",
  ...
  {
    key: "total_branches" as keyof Stats,
    label: "Filiais",
    bg: "bg-pink-50",
    iconBg: "bg-pink-500",
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {statCards.map(({ key, label, bg, iconBg, icon }) => (
          <div key={key} className={`${bg} rounded-2xl border border-slate-200 p-5 shadow-sm`}>
            <div className={`h-11 w-11 rounded-2xl ${iconBg} flex items-center justify-center mb-3`}>
              {icon}
            </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100">
          ...
          <Link href="/devices" className="text-xs font-semibold text-blue-600 hover:text-blue-700">
            Ver todos →
          </Link>
        </div>
              ...
              <Link key={d.id} href="/devices"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 font-mono truncate">
                    {d.ip_address ?? d.rustdesk_id}
                    {d.os ? ` · ${d.os}` : ""}
                  </p>
                </div>
                <span className="bg-emerald-100 text-emerald-600 text-xs font-semibold px-2.5 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                  Online ✓
                </span>
              </Link>
            ))}
